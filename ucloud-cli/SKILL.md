...
- 要求执行环境中安装 `ucloud` CLI。
- 要求在 `~/.ucloud/` 下至少存在一个可用的 CLI profile。

如果 CLI 或其它必要依赖未安装，则停止；先检查操作系统和架构，再征得用户同意并选择对应安装方式。Linux 优先读取 `/etc/os-release` 并结合 `uname -m`，macOS 使用 `sw_vers` 和 `uname -m`。

**安装方法**
- macOS：按架构从 `https://ucloud-infra.cn-bj.ufileos.com/cli/darwin_{amd64,arm64}/ucloud` 下载二进制文件，在用户未指定安装路径的情况下，必需安装到 `$PATH` 中的目录，如果没有权限申请用户授权。
- Linux：按架构从 `https://ucloud-infra.cn-bj.ufileos.com/cli/linux_{amd64,arm64}/ucloud` 下载二进制文件，在用户未指定安装路径的情况下，必需安装到 `$PATH` 中的目录，如果没有权限申请用户授权。
- Windows：从 `https://ucloud-infra.cn-bj.ufileos.com/cli/windows_amd64/ucloud.exe` 下载二进制文件，在用户未指定安装路径的情况下，必需安装到 `PATH` 中的目录，如果没有权限申请用户授权。
