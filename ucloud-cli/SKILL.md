...
**安装方法**
1. 先确认平台、CPU 架构和当前版本；`x86_64`/`amd64` 使用 `amd64` 包，`arm64`/`aarch64` 使用 `arm64` 包。
2. macOS：使用 Homebrew 管理。先用 `brew --version` 确认 Homebrew 可用；如果未安装 Homebrew，先提示用户将按 `https://brew.sh/` 官方安装命令安装 Homebrew，并征得用户授权后再执行安装。Homebrew 可用后，未安装 `ucloud` 时运行 `brew install ucloud`；版本过低时运行 `brew upgrade ucloud`。
3. Linux：从 `https://github.com/ucloud/ucloud-cli/releases` 找到最新 release，下载匹配架构的 `ucloud-linux_{amd64,arm64}.zip` 并解压，使用解压后的 `bin/ucloud` 作为可执行二进制文件，并将其放入 `$PATH` 中的目录或建立链接。
4. Windows：从 `https://github.com/ucloud/ucloud-cli/releases` 找到最新 release，下载匹配架构的 `ucloud-windows_{amd64,arm64}.zip` 并解压，将解压后的 `bin/ucloud` 安装为 `ucloud.exe`，并将其放入 `PATH` 中的目录或建立链接。
5. 如果目标安装目录没有权限，先申请用户授权；不要把二进制文件留在仅当前临时目录可用的位置。
6. 安装或升级完成后运行 `ucloud --version` 或 `ucloud --help`；只有命令可用且版本不低于 `v0.3.1`，才继续资源操作。
